...
**Pixel sprites:** Pixel-art sprite sheets live in `public/sprites/`. `shaman-idle-sheet.png` is a horizontal 4-frame idle sheet (124×124 per frame, 496×124 total). When replacing a sheet, keep the frame dimensions and any `background-size` / `steps()` animation in sync.
